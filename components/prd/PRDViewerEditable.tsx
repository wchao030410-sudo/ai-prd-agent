...
import { CheckCircle2, Users, Target, Zap, FileText, Edit3 } from 'lucide-react';
import { InlineEditPanel } from './InlineEditPanel';
import { safeMap } from '@/lib/utils';

interface EditablePRDViewerProps {
  prd: PRDDocument;
  sessionId: string;
  onPRDUpdate: (updatedPRD: PRDDocument) => void;
}

export function EditablePRDViewer({ prd, sessionId, onPRDUpdate }: EditablePRDViewerProps) {
  ...
  const handleTextSelection = useCallback((field: string) => {
    ...
    <>
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-2 rounded-lg border border-primary/20 bg-primary/5 p-3 text-sm text-primary">
          ...
          <span>选中任意文本后，点击弹出的编辑按钮进行对话式修改</span>
        </div>
          ...
          <Card>
            ...
            <CardContent>
              <p
                ...
                onMouseUp={() => handleTextSelection('background')}
              >
                {prd.background}
              </p>
            </CardContent>
              ...
              <div className="space-y-2">
                {safeMap(prd.targetUsers?.primary, (user, i) => (
                  <div
                    ...
                    onMouseUp={() => handleTextSelection('targetUsers.primary')}
                  >
                    {user}
                  </div>
                ))}
                ...
                <div className="space-y-2">
                  {safeMap(prd.targetUsers?.secondary, (user, i) => (
                    <div
                      key={i}
                      className="rounded-lg bg-muted p-3"
                      onMouseUp={() => handleTextSelection('targetUsers.secondary')}
                    >
                      {user}
                    </div>
                  ))}
              ...
              <ul className="space-y-2">
                {safeMap(prd.painPoints, (pain, i) => (
                  <li
                    ...
                    onMouseUp={() => handleTextSelection('painPoints')}
                  >
              ...
              <ul className="space-y-2">
                {safeMap(prd.coreValue, (value, i) => (
                  <li
                    key={i}
                    className="flex items-start gap-2"
                    onMouseUp={() => handleTextSelection('coreValue')}
                  >
            ...
            <div className="space-y-4">
              {safeMap(prd.features, (feature) => (
                <div
                  ...
                  onMouseUp={() => handleTextSelection('features')}
                >
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {safeMap(prd.competitors, (competitor, i) => (
                <div
                  key={i}
                  className="rounded-lg border p-4"
                  onMouseUp={() => handleTextSelection('competitors')}
                >
                  <h4 className="mb-2 font-semibold">{competitor.name}</h4>
                  <div className="mb-2">
                    <h5 className="mb-1 text-sm font-semibold">核心功能</h5>
                    {(competitor?.features?.length || 0) > 0 ? (
                      <div className="flex flex-wrap gap-1">
                        {safeMap(competitor?.features, (feature: string, j: number) => (
                      ...
                      <p className="text-sm text-muted-foreground">暂无核心功能信息</p>
                    )}
                  </div>
                  <p className="text-sm text-muted-foreground">
      ...
      {selection && selection.rect && (
        <InlineEditPanel
          ...
          targetField={selection.field}
          position={{
            x: selection.rect.left + window.scrollX,
            y: selection.rect.top + window.scrollY,
          }}
          selectionRect={selection.rect}
          onConfirm={(updatedPRD) => {
